...
MEF model filenames follow the pattern `aaa_bb_c.mef` where each component has a specific meaning. Some models use
descriptive names instead of numeric prefixes (e.g. `igiagent_1.mef`, `forest_tower_1.mef`).
...
The `c` value is a Level of Detail index. The engine loads the base model (`c=1`) and substitutes lower-detail versions
at increasing distances, as defined by `ModelLODSettings` in objects.qsc (LOD 2 through LOD 5 plus a cutoff distance).

Not all models have 5 LOD levels -- some only have `c=1`, while others go up to `c=5`. File sizes decrease with higher
`c` values (a median ratio of lowest to highest LOD is ~0.16).
...
Extracted from `magicobjconfig.qvm`, `weapons/weapon.qvm`, `lod.qvm`, and 24 level `objects.qvm` files inside `.res`
archives.
...
Special interactive behaviors assigned to model subparts in `magicobjconfig.qvm`:
...
- `magicobjconfig.qvm` -- MagicObj type definitions per model subpart
...
- `docs/igi2/level.md` (objects.qsc section) -- `ModelLODSettings` confirming LOD system
