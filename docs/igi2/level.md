...
An IGI 2 level's scene graph lives in its `objects.qsc` file (decompiled from `objects.qvm`). Related per-level data — terrain, AI graphs, vegetation, and lightmaps — is documented under the [formats/](formats/) pages.
